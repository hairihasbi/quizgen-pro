...
const QuizViewer: React.FC<QuizViewerProps> = ({ quiz, onClose }) => {
  ...
  // Helper untuk memetakan C1-C6 ke Level 1, 2, 3
  const getCognitiveLevelLabel = (level: string) => {
    const l = level.toUpperCase();
    if (l.includes('C1') || l.includes('C2')) return 'L1 (Pemahaman)';
    if (l.includes('C3')) return 'L2 (Aplikasi)';
  ...
  const handleDownloadPdfSSR = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${exportMode.toUpperCase()}_${quiz.title.replace(/\s+/g, '_')}_Engine.pdf`;
      ...
      <div className="flex-1 overflow-y-auto p-0 md:p-12 flex justify-center custom-scrollbar print-scroll-container">
        <div id="quiz-print-area" className={`print-container bg-white text-gray-900 shadow-none border-none ${exportMode === 'kisi-kisi' ? 'landscape-mode' : ''}`}>
          ...
          <div className="text-center mb-1">
            <h1 className="text-xl font-black m-0">BANK SOAL KURIKULUM MERDEKA</h1>
            <h2 className="text-lg font-bold m-0">{quiz.subject.toUpperCase()} - {quiz.grade.toUpperCase()}</h2>
            <div className="text-[9pt] font-medium tracking-widest text-gray-400 uppercase mt-1">Naskah Evaluasi Hasil Belajar</div>
          </div>
          ...
          <table className="w-full mb-8 text-[10.5pt] font-bold border-collapse">
            <tbody>
              <tr>
                <td className="w-40 py-1">Mata Pelajaran</td><td className="w-4 py-1 text-center">:</td><td className="py-1">{quiz.subject}</td>
                <td className="w-40 py-1 text-right">Alokasi Waktu</td><td className="w-4 py-1 text-center">:</td><td className="w-32 py-1">90 Menit</td>
              </tr>
              <tr>
                <td className="py-1">Jenjang / Kelas</td><td className="py-1 text-center">:</td><td className="py-1">{quiz.level} / {quiz.grade}</td>
                <td className="py-1 text-right">Jumlah Soal</td><td className="py-1 text-center">:</td><td className="py-1">{quiz.questions.length} Butir</td>
              </tr>
              <tr>
                <td className="py-1">Materi Pokok</td><td className="py-1 text-center">:</td><td className="py-1" colSpan={4}>{quiz.topic}</td>
              </tr>
            </tbody>
          </table>

          <div className="text-[11pt] font-black underline uppercase mb-6 text-center">
            {exportMode === 'kisi-kisi' ? 'MATRIKS KISI-KISI PENULISAN SOAL' : 'NASKAH BUTIR SOAL EVALUASI'}
          </div>
                  ...
                  {quiz.questions.map((q, i) => (
                    <tr key={q.id}>
                      ...
                      <td className="border-2 border-black p-2 text-center font-bold">
                        {getCognitiveLevelLabel(q.cognitiveLevel).split(' ')[0]}
                      </td>
                    ...
                    <div className="flex gap-4 text-[11pt] leading-relaxed">
                      ...
                      <div className="flex-1">
                        ...
                        {exportMode === 'soal' && !showAnswer && (
                          <div className="border-b border-dotted border-gray-300 h-1 w-full mt-4 opacity-30"></div>
                        )}
                      </div>
                    </div>
                  </div>
