  ...
  const handleExportPdfClient = async () => {
    ...
    const opt = {
      ...
      pagebreak: { 
        ...
        avoid: ['.pdf-header-group', '.pdf-block', 'tr', 'mjx-container', 'img'] 
      }
    };
  ...
  return (
    <div className="fixed inset-0 bg-white md:bg-gray-900/60 backdrop-blur-3xl z-[500] flex flex-col p-4 md:p-8 animate-in zoom-in-95 duration-300 print-modal-wrapper" role="dialog">
      ...
      <div className="flex-1 overflow-y-auto p-0 md:p-12 flex justify-center custom-scrollbar print-scroll-container">
        <div id="quiz-print-area" className={`print-container bg-white text-gray-900 shadow-none border-none ${exportMode === 'kisi-kisi' ? 'landscape-mode' : ''}`} style={{ backgroundColor: '#ffffff' }}>
                  ...
                  <tbody>
                    {sortedQuestions.map((q, i) => (
                      <tr key={q.id}>
                        ...
                        <td className="border-2 border-black p-2 align-top text-justify">{q.competency || '-'}</td>
                        ...
                        <div key={q.id} className="pdf-block" style={{ pageBreakInside: 'avoid' }}>
                          ...
                          <div className="flex gap-4 text-[11pt] leading-relaxed">
                            ...
                            <div className="flex-1">
                              <div className="mb-2 text-justify" dangerouslySetInnerHTML={{ __html: q.text }}></div>
                              
                              {/* RENDER GAMBAR SOAL DISINI */}
                              {q.image && (
                                <div className="my-4 flex justify-start">
                                  <img src={q.image} className="max-w-[120mm] rounded-lg border border-gray-200 shadow-sm" alt="Stimulus Visual" />
                                </div>
                              )}
                              ...
                              {(exportMode === 'lengkap' || showAnswer) && (
                                <div className="bg-emerald-50 border-2 border-emerald-200 p-4 rounded-2xl text-[9.5pt] italic mt-2 shadow-sm" style={{ pageBreakInside: 'avoid' }}>
                                  <div className="flex items-center gap-3 mb-1">
                                    ...
                                    {q.citation && <span className="bg-blue-600 text-white px-3 py-0.5 rounded-full text-[8pt] font-black not-italic uppercase tracking-widest">🔍 Sitasi: {q.citation}</span>}
                                  </div>
