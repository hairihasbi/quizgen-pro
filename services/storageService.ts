...
export const StorageService = {
  ...
  createToken: async (user: User): Promise<string> => {
    ...
    const payload = StorageService.base64UrlEncode(JSON.stringify({ id: user.id, username: user.username, role: user.role, exp: Math.floor(Date.now() / 1000) + (24 * 60 * 60) }));
  ...
  hashPassword: async (password: string): Promise<string> => {
    ...
    const hash = await window.crypto.subtle.deriveBits({ name: "PBKDF2", salt: salt as any, iterations: iterations, hash: "SHA-256" }, keyMaterial, 256);
  ...
  verifyPassword: async (password: string, storedHash: string): Promise<boolean> => {
    try {
      if (!storedHash || !storedHash.startsWith('v1$')) return btoa(password) === storedHash;
      const [version, saltHex, iterationsStr, hashHex] = storedHash.split('$');
      const salt = StorageService.hexToBuffer(saltHex);
      const iterations = parseInt(iterationsStr);
      const encoder = new TextEncoder();
      const keyMaterial = await window.crypto.subtle.importKey("raw", encoder.encode(password), "PBKDF2", false, ["deriveBits", "deriveKey"]);
      const derivedBits = await window.crypto.subtle.deriveBits({ name: "PBKDF2", salt: salt as any, iterations: iterations, hash: "SHA-256" }, keyMaterial, 256);
  ...
  setCloudConfig: (url: string, token: string) => { localStorage.setItem(CONFIG_KEY, JSON.stringify({ url, token })); _client = null; },
  getClient: () => {
  ...
  init: async () => {
    ...
    if (!client) { _isLocal = true; if (StorageService.localGet('users').length === 0) { const defaults = await createDefaultUsers(); StorageService.localSet('users', defaults); } return true; }
    try {
      await client.batch([
      ...
      if (Number(userCheck.rows[0].count) === 0) {
        ...
        await client.batch(defaults.map(u => ({ sql: "INSERT INTO users (id, username, fullName, role, credits, isActive, email, status, password, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", args: [u.id, u.username, u.fullName, u.role, u.credits, 1, u.email, u.status, u.password, u.createdAt] })), "write");
      }
      await StorageService.syncFull();
      return true;
    } catch (e) { _isLocal = true; return true; }
  },
  syncFull: async () => {
    const client = StorageService.getClient();
  ...
  getApiKeys: async (): Promise<ApiKey[]> => {
    ...
    try { const res = await client.execute("SELECT * FROM api_keys"); return res.rows.map((row: any) => ({ id: row.id, key: row.key, usageCount: Number(row.usage_count), lastUsed: row.last_used, isActive: Boolean(row.isActive), errorCount: Number(row.error_count || 0), lastErrorAt: row.last_error_at })); } catch(e) { return StorageService.localGet('api_keys'); }
  },
  ...
  saveEmailSettings: async (settings: EmailSettings) => {
    const client = StorageService.getClient();
    if (settings.apiKey.includes('•')) { const currentRes = await client?.execute("SELECT data FROM email_settings WHERE id = 'global'"); if (currentRes && currentRes.rows.length > 0) { const oldData = JSON.parse(currentRes.rows[0].data as string); settings.apiKey = oldData.apiKey; } }
    localStorage.setItem('quizgen_email_settings', JSON.stringify(settings));
    if (client && !_isLocal) { try { await client.execute({ sql: "INSERT OR REPLACE INTO email_settings (id, data) VALUES ('global', ?)", args: [JSON.stringify(settings)] }); } catch (e) {} }
  },
  ...
  incrementApiKeyUsage: async (key: string) => {
    ...
    if (idx > -1) { local[idx].usageCount += 1; local[idx].lastUsed = new Date().toISOString(); StorageService.localSet('api_keys', local); }
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE api_keys SET usage_count = usage_count + 1, last_used = ? WHERE key = ?", args: [new Date().toISOString(), key] }); } catch (e) {} }
  },
  reportApiKeyError: async (id: string) => {
    const local = StorageService.localGet('api_keys');
    const idx = local.findIndex((k: any) => k.id === id);
    if (idx > -1) { local[idx].errorCount += 1; local[idx].lastErrorAt = new Date().toISOString(); StorageService.localSet('api_keys', local); }
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE api_keys SET error_count = error_count + 1, last_error_at = ? WHERE id = ?", args: [new Date().toISOString(), id] }); } catch (e) {} }
  },
  resetApiKeyUsage: async (id: string) => {
    const local = StorageService.localGet('api_keys');
    const idx = local.findIndex((k: any) => k.id === id);
    if (idx > -1) { local[idx].usageCount = 0; local[idx].errorCount = 0; StorageService.localSet('api_keys', local); }
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE api_keys SET usage_count = 0, error_count = 0 WHERE id = ?", args: [id] }); } catch (e) {} }
  },
  toggleApiKeyStatus: async (id: string, status: boolean) => {
    const local = StorageService.localGet('api_keys');
    const idx = local.findIndex((k: any) => k.id === id);
    if (idx > -1) { local[idx].isActive = status; StorageService.localSet('api_keys', local); }
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE api_keys SET isActive = ? WHERE id = ?", args: [status ? 1 : 0, id] }); } catch (e) {} }
  },
  deleteApiKey: async (id: string) => {
    const local = StorageService.localGet('api_keys');
    StorageService.localSet('api_keys', local.filter((k: any) => k.id !== id));
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "DELETE FROM api_keys WHERE id = ?", args: [id] }); } catch(e){} }
  },
  ...
  saveQuizzes: async (quizzes: Quiz[]) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.batch(quizzes.map(q => ({ sql: `INSERT OR REPLACE INTO quizzes (id, title, subject, level, grade, topic, subTopic, difficulty, questions, grid, tags, authorId, authorName, isPublished, createdAt, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, args: [q.id, q.title, q.subject, q.level, q.grade, q.topic, q.subTopic || '', q.difficulty, JSON.stringify(q.questions), q.grid, JSON.stringify(q.tags || []), q.authorId, q.authorName || '', q.isPublished ? 1 : 0, q.createdAt, q.status] })), "write"); } catch (e) {} }
  },
  deleteQuiz: async (id: string) => {
    const local = StorageService.localGet('quizzes');
    StorageService.localSet('quizzes', local.filter((q: any) => q.id !== id));
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "DELETE FROM quizzes WHERE id = ?", args: [id] }); } catch(e) {} }
  },
  getUsers: async (): Promise<User[]> => {
    const client = StorageService.getClient();
    if (!client || _isLocal) return StorageService.localGet('users');
    try { const res = await client.execute("SELECT id, username, fullName, role, credits, isActive, email, status, password, createdAt FROM users ORDER BY createdAt DESC"); return res.rows.map((row: any) => ({ id: row.id, username: row.username, fullName: row.fullName, role: row.role as UserRole, credits: Number(row.credits), isActive: Boolean(row.isActive), email: row.email, status: row.status as UserStatus, password: row.password, createdAt: row.createdAt })); } catch(e) { return StorageService.localGet('users'); }
  },
  updateUser: async (userId: string, data: Partial<User>) => {
    const users = StorageService.localGet('users');
    const idx = users.findIndex((u: any) => u.id === userId);
    if (idx > -1) { users[idx] = { ...users[idx], ...data }; StorageService.localSet('users', users); }
    const client = StorageService.getClient();
    if (client && !_isLocal) { const sets: string[] = []; const args: any[] = []; Object.entries(data).forEach(([key, val]) => { sets.push(`${key} = ?`); args.push(typeof val === 'boolean' ? (val ? 1 : 0) : val); }); args.push(userId); try { await client.execute({ sql: `UPDATE users SET ${sets.join(', ')} WHERE id = ?`, args: args }); } catch (e) {} }
  },
  saveUsers: async (users: User[]) => {
    StorageService.localSet('users', users);
    const client = StorageService.getClient();
    if (!client || _isLocal) return;
    try { await client.batch(users.map(u => ({ sql: "INSERT OR REPLACE INTO users (id, username, fullName, role, credits, isActive, email, status, password, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", args: [u.id, u.username, u.fullName || '', u.role, Number(u.credits), u.isActive ? 1 : 0, u.email || '', u.status || 'approved', u.password || '', u.createdAt || new Date().toISOString()] })), "write"); } catch(e){}
  },
  deleteUser: async (id: string) => {
    const local = StorageService.localGet('users');
    StorageService.localSet('users', local.filter((u: any) => u.id !== id));
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "DELETE FROM users WHERE id = ?", args: [id] }); } catch(e) { } }
  },
  ...
  addLog: async (log: QuizLog) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "INSERT INTO logs (id, timestamp, category, action, details, status, userId, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", args: [log.id, log.timestamp, log.category, log.action, log.details, log.status, log.userId, log.metadata || ''] }); } catch(e){} }
  },
  ...
  updateUserCredits: async (userId: string, amount: number) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE users SET credits = credits + ? WHERE id = ?", args: [amount, userId] }); } catch(e){} }
  },
  toggleQuizPublication: async (quizId: string, status: boolean) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE quizzes SET isPublished = ? WHERE id = ?", args: [status ? 1 : 0, quizId] }); } catch(e){} }
  },
  getEmails: async (email?: string): Promise<EmailNotification[]> => {
    const client = StorageService.getClient();
    let emails: EmailNotification[] = [];
    if (!client || _isLocal) { emails = StorageService.localGet('emails'); } else { try { const res = await client.execute("SELECT * FROM emails ORDER BY timestamp DESC LIMIT 50"); emails = res.rows.map((row: any) => ({ id: row.id, to: row.to_addr, subject: row.subject, body: row.body, type: row.type as any, timestamp: row.timestamp, isRead: Boolean(row.isRead) })); } catch(e) { emails = StorageService.localGet('emails'); } }
  ...
  getEmailsPaged: async (email: string, page: number = 1, limit: number = 10): Promise<{ emails: EmailNotification[], hasMore: boolean }> => {
    ...
    if (!client || _isLocal) { const all = StorageService.localGet('emails').filter((e: any) => e.to === email); const slice = all.slice(offset, offset + limit); return { emails: slice, hasMore: all.length > offset + limit }; }
    try { const res = await client.execute({ sql: "SELECT * FROM emails WHERE to_addr = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?", args: [email, limit + 1, offset] }); const rows = res.rows.map((row: any) => ({ id: row.id, to: row.to_addr, subject: row.subject, body: row.body, type: row.type as any, timestamp: row.timestamp, isRead: Boolean(row.isRead) })); const hasMore = rows.length > limit; return { emails: hasMore ? rows.slice(0, limit) : rows, hasMore }; } catch(e) { return { emails: [], hasMore: false }; }
  },
  addEmail: async (email: EmailNotification) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "INSERT INTO emails (id, to_addr, subject, body, type, timestamp, isRead) VALUES (?, ?, ?, ?, ?, ?, ?)", args: [email.id, email.to, email.subject, email.body, email.type, email.timestamp, email.isRead ? 1 : 0] }); } catch(e){} }
  },
  markEmailAsRead: async (id: string) => {
    ...
    const client = StorageService.getClient();
    if (client && !_isLocal) { try { await client.execute({ sql: "UPDATE emails SET isRead = 1 WHERE id = ?", args: [id] }); } catch(e){} }
  },
  deleteEmails: async (ids: string[]) => {
    const local = StorageService.localGet('emails');
    StorageService.localSet('emails', local.filter((e: any) => !ids.includes(e.id)));
    const client = StorageService.getClient();
    if (client && !_isLocal) {
      try {
        const placeholders = ids.map(() => '?').join(',');
        await client.execute({ sql: `DELETE FROM emails WHERE id IN (${placeholders})`, args: ids });
      } catch (e) {}
    }
  },
