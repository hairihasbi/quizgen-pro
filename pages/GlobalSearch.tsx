...
const GlobalSearch: React.FC = () => {
  ...
  const allAvailableTags = useMemo(() => {
    ...
    allQuizzes.forEach(q => {
      if (q.tags) q.tags.forEach(t => tagSet.add(t));
    });
      ...
      <div className="bg-white p-12 rounded-[3.5rem] border shadow-sm space-y-10">
        <div className="flex flex-col md:flex-row justify-between items-start gap-8">
           ...
           <div className="relative w-full md:w-96 group">
              <input 
                type="text" 
                placeholder="Cari materi, judul, atau konsep..."
                className="w-full pl-14 pr-6 py-5 rounded-[2.5rem] bg-gray-50 border-2 border-transparent focus:border-orange-500 focus:bg-white outline-none shadow-inner font-bold text-gray-800 transition-all"
                value={searchTerm}
        ...
        <div className="space-y-4">
           ...
           <div className="flex flex-wrap gap-2">
              <button 
                ...
                className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${!selectedTag ? 'orange-gradient text-white shadow-lg' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}
              >SEMUA</button>
              {allAvailableTags.map(tag => (
                <button 
                  key={tag}
                  onClick={() => setSelectedTag(tag === selectedTag ? null : tag)}
                  className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border-2 ${selectedTag === tag ? 'bg-orange-50 text-orange-600 border-orange-500 shadow-sm' : 'bg-white text-gray-400 border-gray-100 hover:border-orange-200'}`}
                >
                  {tag}
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
           {filteredResults.map(q => (
             <div 
               ...
               className="bg-white p-10 rounded-[3rem] border border-orange-50 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 cursor-pointer group flex flex-col"
             >
                ...
                <div className="flex-1 flex flex-wrap gap-1 mb-8">
                   {q.tags && q.tags.map(t => (
                     <span key={t} className="text-[8px] font-bold text-orange-400 bg-orange-50/30 px-2 py-0.5 rounded-full">{t}</span>
                   ))}
                </div>
