...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  if (loading) return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="bg-white p-10 rounded-[3rem] h-48 orange-gradient flex items-center justify-center relative overflow-hidden" aria-hidden="true">
        ...
        <span className="text-white font-black uppercase tracking-[0.3em] relative z-10">Neural Synchronization...</span>
      </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
        ...
        <div className="relative z-10 text-center md:text-left space-y-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center md:justify-start items-center">
            {user.role === UserRole.TEACHER && (
               <button onClick={() => setShowTopUp(true)} className="px-8 py-3.5 bg-white text-orange-600 font-black rounded-2xl text-[10px] uppercase shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-2 focus:ring-4 focus:ring-white/50">⚡ Top Up Kredit AI</button>
            )}
            {isAdmin && lastCron && (
              <div className="px-5 py-3 bg-emerald-500/20 backdrop-blur-md rounded-2xl border border-emerald-400/30 flex items-center gap-3">
                 ...
                 <span className="text-[9px] font-black uppercase tracking-widest text-emerald-100">Worker Active: {new Date(lastCron.timestamp).toLocaleTimeString()}</span>
              </div>
            )}
            <div className="px-6 py-3 bg-black/20 backdrop-blur-md rounded-2xl text-[9px] font-bold flex items-center gap-2 border border-white/10 uppercase tracking-widest" role="status">
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-8 rounded-[3rem] border border-orange-50 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
            <div className="relative z-10">
              <div className={`w-14 h-14 ${stat.color} rounded-2xl flex items-center justify-center text-3xl mb-6 shadow-inner`} aria-hidden="true">{stat.icon}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-white rounded-[3rem] border border-orange-100 shadow-sm p-10 space-y-8 flex flex-col">
          ...
          <div className="space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
             {inbox.slice(0, 10).map((n) => (
               <div key={n.id} onClick={() => handleReadMail(n)} className={`p-5 rounded-2xl border transition-all cursor-pointer hover:scale-[1.02] active:scale-95 animate-in slide-in-from-top-2 duration-300 ${n.isRead ? 'bg-gray-50 border-transparent opacity-60' : 'bg-orange-50 border-orange-200 shadow-sm'}`}>
             ...
             {inbox.length === 0 && <div className="py-20 text-center text-gray-300 font-bold uppercase text-[9px] tracking-widest">Inbox Kosong</div>}
          </div>
        </div>

        <div className="lg:col-span-2 bg-white rounded-[3rem] border border-orange-100 shadow-sm overflow-hidden flex flex-col">
            ...
            {transactions.map((trx) => (
              <div key={trx.id} className="p-8 flex items-center justify-between hover:bg-orange-50/10 transition-all group">
                <div className="flex items-center gap-5">
                   <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${trx.status === 'SUCCESS' ? 'bg-emerald-50 text-emerald-500' : trx.status === 'PENDING' ? 'bg-amber-50 text-amber-500' : 'bg-rose-50 text-rose-500'}`}>{trx.status === 'SUCCESS' ? '💵' : trx.status === 'PENDING' ? '⏳' : '⚠️'}</div>
                   <div>
                ...
                <div className="flex items-center gap-6">
                   <div className="text-right">
                      ...
                      <div className={`text-[9px] font-black uppercase px-3 py-1 rounded-lg inline-block mt-1 ${trx.status === 'SUCCESS' ? 'bg-emerald-500 text-white' : 'bg-amber-100 text-amber-600'}`}>{trx.status}</div>
                   </div>
                   <button onClick={(e) => { e.stopPropagation(); handleDeleteTransaction(trx.id, trx.externalId); }} className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all opacity-0 group-hover:opacity-100" title="Hapus riwayat">🗑️</button>
                </div>
              </div>
            ))}
            {transactions.length === 0 && <div className="py-20 text-center text-gray-300 font-bold uppercase text-[9px] tracking-widest">Belum Ada Transaksi</div>}
          </div>
        </div>
      </div>
      {/* Modals keep same */}
      {selectedMail && (
        <div className="fixed inset-0 bg-gray-900/80 backdrop-blur-xl z-[700] flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-500">
              ...
              <div className="p-10 text-gray-700 leading-relaxed text-sm whitespace-pre-wrap">{selectedMail.body}</div>
