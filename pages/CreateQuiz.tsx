...
const CreateQuiz: React.FC<CreateQuizProps> = ({ user, onSuccess }) => {
  ...
  const saveToDatabase = async () => {
    ...
    try {
      ...
      setErrorMsg("Koneksi Database Terputus saat menyimpan gambar besar.");
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-orange-100">
           <div className="flex flex-col md:flex-row justify-between items-center mb-10 border-b pb-8 gap-6">
              ...
              <div className="flex gap-4">
                 ...
                 <button onClick={saveToDatabase} disabled={isSaving} className="px-12 py-4 orange-gradient text-white font-black rounded-2xl shadow-xl text-[10px] uppercase hover:scale-105 transition-all flex items-center gap-2">
              ...
              {workbenchData.questions.map((q, idx) => (
                <div key={q.id} className="p-10 bg-gray-50/40 rounded-[3.5rem] border-2 border-gray-100">
                  ...
                  <div className="space-y-8">
                    ...
                    {q.image && (
                      <div className="flex justify-center bg-white p-6 rounded-[2.5rem] border shadow-inner">
                        <img src={q.image} className="max-w-full md:max-w-md rounded-2xl shadow-2xl" alt="Ilustrasi" />
                      </div>
                    )}
                    {q.options && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {q.options.map(opt => (
                          <div key={opt.label} className="p-6 rounded-[2rem] bg-white border-2 border-transparent hover:border-orange-200 transition-all flex items-center gap-5">
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-orange-100 flex flex-col md:flex-row gap-8 items-center justify-between">
        ...
        <div className="bg-orange-50/50 px-8 py-5 rounded-[2rem] border border-orange-100">
            <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mb-1 text-center">Kredit AI</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {errorMsg ? (
                ...
                <div className="space-y-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Judul Dokumen</label>
                    <input type="text" className="w-full px-8 py-6 rounded-[2rem] bg-gray-50 border-2 border-transparent focus:border-orange-500 focus:bg-white outline-none font-bold text-gray-800 transition-all" placeholder="E.g. Ulangan Harian Fisika" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
                  </div>
                  
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Jenjang</label>
                       <select className="w-full px-8 py-6 rounded-[2rem] bg-gray-50 font-bold outline-none" value={formData.level} onChange={e => setFormData({...formData, level: e.target.value})}>
                    ...
                    <div className="space-y-2">
                       ...
                       <select className="w-full px-8 py-6 rounded-[2rem] bg-gray-50 font-bold outline-none" value={formData.grade} onChange={e => setFormData({...formData, grade: e.target.value})}>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Topik Utama</label>
                    <input type="text" className="w-full px-8 py-6 rounded-[2rem] bg-gray-50 border-2 border-transparent focus:border-orange-500 font-bold outline-none" placeholder="E.g. Fotosintesis" value={formData.topic} onChange={e => setFormData({...formData, topic: e.target.value})} />
                  </div>

                  <div className="p-8 bg-orange-50/50 rounded-[2.5rem] border border-orange-100 flex flex-col md:flex-row justify-between items-center gap-4">
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                           ...
                           <p className="text-[9px] text-gray-400 font-bold">Otomatis generate gambar soal</p>
                        </div>
                     </div>
                     <input type="number" min="0" max={formData.count} className="w-24 px-6 py-4 rounded-2xl bg-white border-2 border-orange-200 text-center font-black text-orange-600" value={formData.imageQuestionsCount} onChange={e => setFormData({...formData, imageQuestionsCount: Math.min(formData.count, parseInt(e.target.value) || 0)})} />
                  </div>
                  ...
                  <button onClick={startGeneration} disabled={isGenerating} className="w-full py-8 orange-gradient text-white font-black rounded-[2.5rem] text-xl shadow-2xl hover:scale-[1.02] active:scale-95 transition-all uppercase flex items-center justify-center gap-4 group disabled:opacity-50">
                    ...
                    {isGenerating ? "SYNTESIZING..." : "GENERATE SOAL AI ➜"}
                  </button>
                </div>
              )}
           </div>
        ...
        <div className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-sm h-fit space-y-8">
           ...
           <div className="space-y-6">
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Total Soal</label>
                 <input type="number" className="w-full px-8 py-5 rounded-2xl bg-gray-50 font-black outline-none" value={formData.count} onChange={e => setFormData({...formData, count: Math.min(50, parseInt(e.target.value) || 1)})} />
              </div>
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Kesulitan</label>
                 <select className="w-full px-8 py-5 rounded-2xl bg-gray-50 font-black outline-none" value={formData.difficulty} onChange={e => setFormData({...formData, difficulty: e.target.value})}>
              ...
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest">Model</label>
        ...
        <div className="fixed inset-0 bg-gray-950/95 backdrop-blur-3xl z-[600] flex items-center justify-center p-6">
           <div className="bg-white w-full max-w-lg rounded-[4rem] p-16 text-center space-y-10 shadow-2xl relative overflow-hidden">
              ...
              <div className="bg-orange-50 p-6 rounded-[2rem] border border-orange-100">
                 <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest">Estimasi Selesai: {remainingSeconds} Detik</p>
              </div>
           </div>
        </div>
      )}
